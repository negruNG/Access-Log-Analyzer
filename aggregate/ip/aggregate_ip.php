...
while (($column = fgetcsv($file, 10000, " ")) !== FALSE) {
			...
			//INSERT with SQL Injection Prevention            
...
//Trim the first character from date ([)		
...
function get_ip_data(){
	...
	//select query
	$query = "select ip_address, count(ip_address) as cnt_ip from log_entry where STR_TO_DATE(log_date , '%d/%b/%Y:%H:%i:%s') >= '".$dt_start."' group by ip_address;";
	
	$result = mysqli_query($conn, $query);
	
	$query_between = "select ip_address, count(ip_address) as cnt_ip from log_entry where 
	(STR_TO_DATE(log_date , '%d/%b/%Y:%H:%i:%s') between '".$dt_start."' and '".$dt_end."') group by ip_address;";
	$result_between = mysqli_query($conn, $query_between);
	
	$table_data = array();
	
    //check if only start date is entered...	
...
if(file_put_contents($file_name, get_ip_data())){
