...
function get_url_data(){
	...
	$query = "select url, count(url) as cnt_url from log_entry where STR_TO_DATE(log_date , '%d/%b/%Y:%H:%i:%s') = '".$dt_start."' group by url;";
	$result = mysqli_query($conn, $query);
		
	$query_between = "select url, count(url) as cnt_url from log_entry where 
	(STR_TO_DATE(log_date , '%d/%b/%Y:%H:%i:%s') between '".$dt_start."' and '".$dt_end."') group by url;";
...
echo '<pre>';
print_r(get_url_data());
echo '</pre>';

//create JSON file on file system
$file_name = date('d-m-Y').'.json';
if(file_put_contents($file_name, get_url_data())){
